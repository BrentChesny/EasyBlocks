...
/**
 * @brief The MultiplyBlock class
 *
 * @author Brent Chesny
 */
class MultiplyBlock : public ExpressionBlock
{
public:
    /**
     * @brief Constructor
     */
    MultiplyBlock() : _left(NULL), _right(NULL) {}

    /**
     * @brief Destructor
     */
    virtual ~MultiplyBlock();

    /**
     * @brief Returns a unique id of the block
     * @return A unique id specific to the block
     */
    virtual QString getId() const {return QString("Number_MultiplyBlock");}
     ...
     */
    virtual Block* newInstance() const {return new MultiplyBlock();}
