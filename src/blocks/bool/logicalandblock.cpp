...
LogicalAndBlock::~LogicalAndBlock()
{
    if(_left != NULL)
        delete _left;
    if(_right != NULL)
...
QList<Block::ParamType> LogicalAndBlock::getParamTypes() const
{
...
void LogicalAndBlock::executeNextStep(ExecutionThread& executionThread) const
{
...
bool LogicalAndBlock::addParameter(Block* parameter, int index)
{
